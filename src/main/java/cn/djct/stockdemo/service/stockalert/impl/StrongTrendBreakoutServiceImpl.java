...
import cn.djct.stockdemo.common.StrongTrendBreakoutCalculator;
...
public class StrongTrendBreakoutServiceImpl implements StrongTrendBreakoutService {

    private static final String STRATEGY_TYPE = "STRONG_TREND_BREAKOUT";
    ...
    private final StrongTrendBreakoutCalculator strongTrendBreakoutCalculator;
    ...
    public int selectStocks(LocalDate tradeDate) {
        ...
        //查近129个交易日历日期，按照T-128到T升序排列
        LocalDate startDate = tradeCalendarService.getPreviousTradingDay(tradeDate, 128);
        List<LocalDate> dates = tradeCalendarService.getTradingDays(startDate, tradeDate);
        if (dates.size() != 129 || !dates.get(0).equals(startDate)//避免日期错误
                || !dates.get(128).equals(tradeDate)) {
            throw new IllegalStateException("强势趋势突破需要完整129个交易日历日期");
        }
        ...
        while (true) {
            ...
            for (StockBasic stock : stocks) {
                ...
                if (invalidReason != null) {
                    invalidTodayCount++;
                    log.warn("强势趋势突破跳过当日行情异常股票，stockCode={}，tradeDate={}，reason={}",
                ...
                for (LocalDate date : dates) {
                    ...
                    if (quote == null || !positive(quote.getClosePrice()) || !positive(quote.getHighPrice())
                        ...
                        log.warn("强势趋势突破跳过历史行情异常股票，stockCode={}，tradeDate={}，missingOrInvalidDate={}",
                ...
                // 累计日线须超过60条，含今日最多129条；从已有历史首日到今日必须连续。
                if (ordered.size() < 61 || stockDailyQuoteMapper.countByStockCodeThroughTradeDate(stock.getStockCode(), tradeDate) <= 60) {
                    insufficientCount++;
                    log.info("强势趋势突破历史不足61日，跳过，stockCode={}，availableDays={}", stock.getStockCode(), ordered.size());
                    continue;
                }
                //进行强势趋势突破匹配
                if (strongTrendBreakoutCalculator.matches(ordered)) {
                    selected.add(stock);
                }
            }
        ...
        for (int start = 0; start < selected.size(); start += BATCH_SIZE) {
            //批量插入
            stockSelectionResultMapper.insertResults(tradeDate, STRATEGY_TYPE,
                    selected.subList(start, Math.min(start + BATCH_SIZE, selected.size())));
        }
        //验证处理数量和结果数量一致
        if (processedCount != expectedCount || stockSelectionResultMapper.countResults(tradeDate, STRATEGY_TYPE) != selectedCount) {
            throw new IllegalStateException("强势趋势突破股票处理数量或结果数量不一致");
        }
        //标记任务完成
        stockSelectionRunMapper.completeRun(tradeDate, STRATEGY_TYPE);
        log.info("强势趋势突破计算完成，status={}，tradeDate={}，processedCount={}，selectedCount={}，insufficientCount={}，invalidHistoryCount={}，invalidTodayCount={}",
    ...
    public PageRespVo<LeftSideStockRespVo> findByTradeDate(LocalDate tradeDate, int pageNum, int pageSize) {
        ...
        if (!stockSelectionRunMapper.isCompleted(tradeDate, STRATEGY_TYPE)) {
            throw new IllegalStateException("该交易日强势趋势突破尚无成功结果，任务未执行、执行中或执行失败");
        }
        //获取该任务完成后记录的总数量
