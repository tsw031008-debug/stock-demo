...
/** 按未复权日线判断平台突破，输入为交易日历对齐的60至200日升序行情。 */
@Component
public class PlatformBreakoutCalculator {
    ...
    public boolean matches(List<StockDailyQuote> quotes) {
        if (quotes.size() < 60 || quotes.size() > 200) {
            throw new IllegalArgumentException("平台突破需要60至200个连续交易日行情");
        }
        // 数据验证
        ...
        // 不足200日使用已有连续历史，所有条件相对计算日定位。
        // CON1 : MA30 > MA60
        // MA30 > MA60等价于30日总和×2 > 60日总和，避免均线舍入改变边界。
        if (sumClose(quotes, size - 30, size).multiply(BigDecimal.valueOf(2))
                .compareTo(sumClose(quotes, size - 60, size)) <= 0) {
            return false;
        }
        //CON2:最近存在平台调整，最近10日存在收盘价小于10日均线
        boolean adjusted = false;
        for (int day = size - 10; day < size; day++) {
            //存在
        ...
        BigDecimal platform = highest(quotes, size - 26, size - 5);
        BigDecimal close = quotes.get(size - 1).getClosePrice();
        //上穿：昨收小于次高点，今收大于次高点
        if (quotes.get(size - 2).getClosePrice().compareTo(platform) >= 0
                || close.compareTo(platform) <= 0) {
            return false;
        }
        //CON4:收盘价在200日最高价附近
        //量化条件：收盘价在最近200日最高价上下10%之间。
        BigDecimal highest200 = highest(quotes, 0, size);
