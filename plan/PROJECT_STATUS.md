...
项目处于第一阶段基础能力建设，采用 Spring Boot 模块化单体架构。当前已完成交易日历、股票基础信息、股票日行情、近两年历史日线回补、东方财富沪深A股资金流向每日同步与按日期分页查询、市场水位查询，以及涨速预警和开板提醒接口；下一步进行股票预警真实运行验证。
...
- 新浪历史资金流向全量回补已于2026-08-26 10:27启动，后台进程PID 25864，按单线程1.5秒间隔运行。
...
- 股票预警采用单实例进程内60秒快照，多实例部署时各实例不会共享同一快照。
...
- 新浪全量回补仍在运行，完成前不能视为5211只股票的最近121个交易日已完整覆盖。
...
4. 等待新浪最近121个交易日资金流向全量回补完成，核对日期覆盖、重复记录、来源分布和正式数据保护结果。
