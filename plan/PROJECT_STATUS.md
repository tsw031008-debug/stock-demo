...
项目处于第一阶段基础能力建设，采用 Spring Boot 模块化单体架构。当前已完成交易日历、股票基础信息、股票日行情、近两年历史日线回补，以及东方财富沪深A股资金流向每日同步链路；市场水位业务分析及业务口径确认已完成，可以进入实现。
...
1. 在当日股票基础信息就绪后，验证15:05东方财富资金流向任务并核对入库完整性。
2. 验证下一个交易日15:02腾讯日行情增量任务，并核对与历史数据的幂等衔接。
3. 按已确认业务口径实现市场水位查询接口。
